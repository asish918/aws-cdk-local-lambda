...
async function cmdServe(opts: ServeArgs): Promise<void> {
  ...
  const handle = await createLocalApp({
    ...
    watch: opts.watch,
    onReload: (p, n) =>
      frameworkLog(`[cdk-local] reload (${n} cached) after ${p}`),
    onManifestChange: p =>
      frameworkLog(
...
async function cmdDev(opts: DevArgs): Promise<void> {
  ...
  await listenUntilSignal({
    ...
    onListening: () =>
      logSuccess(
        `[cdk-local] running local server on http://localhost:${port} (${handle.routes.length} routes)`
      ),
    onStop: () => handle.stop()
  });
}
...
program
  ...
  .option(
    '--repo-root <dir>',
    'repo root for resolving handler source paths (default: cwd)'
  )
...
program
  ...
  .option('--quiet', 'suppress framework log output', false)
  .action(
    (o: { manifest: string; port: string; watch?: boolean; quiet?: boolean }) =>
